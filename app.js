...
function getProgress(user) { return JSON.parse(localStorage.getItem(STORAGE_PREFIX + "progress_" + user) || "{}"); }
function saveProgress(user, progress) { localStorage.setItem(STORAGE_PREFIX + "progress_" + user, JSON.stringify(progress)); }
function getScores(user) { return JSON.parse(localStorage.getItem(STORAGE_PREFIX + "scores_" + user) || "{}"); }
function saveScores(user, scores) { localStorage.setItem(STORAGE_PREFIX + "scores_" + user, JSON.stringify(scores)); }
...
function getAcceptableAnswers(answerStr) {
  ...
  for (const part of parts) {
    ...
    if (slashMatch) { answers.add(normalize(slashMatch[1])); answers.add(normalize(slashMatch[1] + slashMatch[2])); }
    if (part.includes("/") && !part.includes("/-")) {
      for (const sub of part.split("/")) answers.add(normalize(sub));
    }
  }
...
function render() {
  if (state.currentUser) localStorage.setItem(STORAGE_PREFIX + "lastUser", state.currentUser);
  switch (state.screen) {
...
function renderWelcome() {
  ...
  document.getElementById("start-btn").addEventListener("click", () => loginUser(input.value));
  input.addEventListener("keydown", e => { if (e.key === "Enter") loginUser(input.value); });
  document.querySelectorAll(".user-item .name").forEach(el => el.addEventListener("click", () => loginUser(el.dataset.user)));
  document.querySelectorAll(".delete-user").forEach(el => el.addEventListener("click", e => {
    e.stopPropagation();
...
function renderPortions() {
  const cards = VOCABULARY.portions.map((p, i) => {
    ...
    function progressRow(label, learned, total, score) {
      ...
      return `<div class="dir-row"><span class="dir-label">${label}</span><div class="progress-bar"><div class="progress-fill${done ? " complete" : ""}" style="width:${pct}%"></div></div><span class="progress-text">${learned}/${total}</span>${scoreText}</div>`;
    }
    return `
  ...
  $app.innerHTML = `
    <div class="top-bar">
      <div class="user-badge"><div class="user-avatar">${esc(state.currentUser.charAt(0).toUpperCase())}</div><span>${esc(state.currentUser)}</span></div>
      <button class="btn btn-ghost btn-small" id="logout-btn">Wechseln</button>
    </div>
    <div class="app-header"><h1>Portionen</h1><div class="subtitle">W\u00e4hle eine Portion</div></div>
    <div class="portions-grid">${cards}</div>`;
  document.getElementById("logout-btn").addEventListener("click", () => { state.currentUser = null; state.screen = "welcome"; render(); });
...
function renderGame() {
  ...
  document.querySelectorAll(".direction-toggle button").forEach(btn => {
    btn.addEventListener("click", () => {
      if (!state.showQuestion && state.answers.length === 0) {
  ...
  overlay.innerHTML = `
    <div class="question-card">
      ...
      <div class="quiz-input-group">
        <input id="q-input" type="text" class="${inputExtra}" placeholder="Deine Antwort..." autocomplete="off" ${state.answered ? "disabled" : ""}>
      </div>
      ${feedbackHtml}
      ${actionsHtml}
    </div>`;
...
function renderResults() {
  ...
  $app.innerHTML = `
    <div class="app-header"><h1>Ergebnis</h1><div class="subtitle">Portion ${portion.portion} &mdash; ${portion.day}, ${portion.date}</div></div>
  ...
  document.getElementById("back-btn").addEventListener("click", () => { state.screen = "portions"; render(); });
  document.getElementById("retry-btn").addEventListener("click", () => startQuiz(state.selectedPortion));
}
...
function init() {
  ...
  if (lastUser && getUsers().includes(lastUser)) { state.currentUser = lastUser; state.screen = "portions"; }
  render();
}
init();
